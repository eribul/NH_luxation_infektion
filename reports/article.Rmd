...
**SETTING:** Register data from Sweden for model derivation and (from Denmark for external) validation.  

**PARTICIPANTS:** `r nmbr(N_shar)` patients from the Swedish Hip Arthroplasty Register, and `r nmbr(N_dk)` patients from the Danish Hip Arthroplasty Register operated with elective THA. 

**MAIN OUTCOME MEASURE:** Logistic LASSO regression with bootstrap ranking was used to develop a risk prediction model for PJI within 90 days based on the Swedish cohort, followed by external validation on the Danish cohort.
...
- Attempts have been made to develop individualized risk prediction models for clinical usage in the US and Australia but none of those models has gained general acceptance due to limited samples, low discriminating ability, sub-optimal calibration or absence of external model validation. 

## What this study adds
- Our prediction model was derived on a Swedish cohort and externally validated on a Danish cohort
- Our model is based on easily accessible data that are available before THA
- Compared to more complex comorbidity indices or to ASA class, our model had superior discriminatory ability and good calibration
- We provide a simple web calculator for easy use in clinical practice (https://erikbulow.shinyapps.io/thamortpred/).
